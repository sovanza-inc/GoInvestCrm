    ...
    <div className="min-h-screen bg-card" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <section className="py-16 px-6 bg-gradient-to-b from-slate-50 to-white">
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-4xl sm:text-5xl font-bold text-foreground mb-4" style={{ fontFamily: 'Outfit, sans-serif' }}>Blogs</h1>
          ...
          <div className="max-w-md mx-auto relative mb-8" data-testid="blog-search">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 rounded-full border border-border bg-card text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400"
        ...
        <section className="px-6 pb-12">
          <div className="max-w-4xl mx-auto">
            <Link to={`/blog/${latestPost.slug}`} className="group block" data-testid="blog-featured-post">
              ...
              <h2 className="text-2xl sm:text-3xl font-bold text-foreground mb-2 group-hover:text-blue-600 transition-colors" style={{ fontFamily: 'Outfit, sans-serif' }}>
                {latestPost.title}
              </h2>
      ...
      {activeCategory === "all" && !searchQuery && featuredPosts.length > 1 && (
        <section className="px-6 pb-16">
          <div className="max-w-4xl mx-auto">
            <h3 className="text-xl font-bold text-foreground text-center mb-8" style={{ fontFamily: 'Outfit, sans-serif' }}>Featured Blog</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {featuredPosts.slice(0, 2).map(post => (
                <Link key={post.slug} to={`/blog/${post.slug}`} className="group" data-testid={`blog-featured-${post.slug}`}>
                  ...
                  <h4 className="text-sm font-bold text-foreground group-hover:text-blue-600 transition-colors leading-snug mb-1">{post.title}</h4>
      ...
      {activeCategory === "all" && !searchQuery && (
        <section className="py-16 px-6 bg-background">
          <div className="max-w-4xl mx-auto">
            <h3 className="text-xl font-bold text-foreground text-center mb-8" style={{ fontFamily: 'Outfit, sans-serif' }}>GoSocial for Industries</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {BLOG_POSTS.filter(p => !p.featured).slice(0, 3).map(post => (
                <Link key={post.slug} to={`/blog/${post.slug}`} className="group" data-testid={`blog-industry-${post.slug}`}>
                  <div className="rounded-xl overflow-hidden border border-border shadow-sm bg-card mb-3">
      ...
      <section className="py-16 px-6">
        <div className="max-w-4xl mx-auto">
          <h3 className="text-xl font-bold text-foreground text-center mb-8" style={{ fontFamily: 'Outfit, sans-serif' }}>
          ...
          {filteredPosts.length === 0 ? (
            <div className="text-center py-12" data-testid="blog-no-results">
              <p className="text-muted-foreground text-base">No articles found. Try a different search or category.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6" data-testid="blog-grid">
              {filteredPosts.map(post => (
                <Link key={post.slug} to={`/blog/${post.slug}`} className="group" data-testid={`blog-card-${post.slug}`}>
                  ...
                  <Badge className="bg-muted text-muted-foreground border-0 text-[10px] px-2.5 py-0.5 mb-2 font-medium">
                    {BLOG_CATEGORIES.find(c => c.key === post.category)?.label}
                  </Badge>
                  <h4 className="text-sm font-bold text-foreground group-hover:text-blue-600 transition-colors leading-snug mb-1">{post.title}</h4>
      ...
      {/* Footer */}
      <footer className="py-12 px-6 bg-[#020617] border-t border-border">
